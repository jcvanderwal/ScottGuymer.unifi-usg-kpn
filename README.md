...
- When connected to **USG**, you're connected to EdgeOS. Double pressing _tab_ will give you an overview of commands.
...
7. Click: _Queue change_.
8. Go to: _Device Management_ and click _Force provision_
...
1. Replace the MAC address placeholder `xx-xx-xx-xx-xx-xx` with the real MAC address of the USG (see _Setup basic Internet, step 2_).
...
2. In (my) case the Controller is running in a **Docker container (without SSH deamon)** with a volume mapping. Connect with SFTP _to the docker host_ and `/volume1/docker/unifi/data/sites/default` and `push config.gateway.json`.
...
1. In the **Unifi Controller** -> Devices -> USG
2. Go to: _Device Management_ and click _Force provision_
...
3. _Reprovision the configuration_
...
2. Check if _IGMP Snooping_ is supported by your switch (and all other switches in between the NTU and the IPTV decoders)
